...
class Localizer():
    def __init__(self,
        ...
        logger.info('Reading the 3D model...')
        if not isinstance(reference_sfm, pycolmap.Reconstruction):
            self.reference_sfm = pycolmap.Reconstruction(reference_sfm)
        self.db_name_to_id = {img.name: i for i,
                              img in self.reference_sfm.images.items()}

        config = {"estimation": {"ransac": {"max_error": ransac_thresh}},
                  **(config or {})}
        self.localizer = QueryLocalizer(self.reference_sfm, config)
    ...
    def loc(self,
        ...
        poses = {}
        retrieval_dict = {}
        for qimg, rimg in retrieval:
            if qimg not in retrieval_dict:
                retrieval_dict[qimg] = [rimg]
            else:
                retrieval_dict[qimg].append(rimg)
        for qname, qcam in tqdm(queries):
            if qname not in retrieval_dict:
                logger.warning(
                    f'No images retrieved for query image {qname}. Skipping...')
            ...
            for n in db_names:
                if n not in self.db_name_to_id:
                    logger.warning(
                        f'Image {n} was retrieved but not in database')
                    continue
                db_ids.append(self.db_name_to_id[n])

            ret, log = self.pose_from_cluster(
                qname, qcam, db_ids, features, matches)
            if ret['success']:
                poses[qname] = (ret['qvec'], ret['tvec'])
            else:
                closest = self.reference_sfm.images[db_ids[0]]
                poses[qname] = (closest.qvec, closest.tvec)
    ...
    def pose_from_cluster(self,
        ...
        num_matches = 0
        for i, db_id in enumerate(db_ids):
            image = self.localizer.reconstruction.images[db_id]
            if image.num_points3D() == 0:
                logger.debug(f'No 3D points found for {image.name}.')
                continue
            points3D_ids = np.array([p.point3D_id if p.has_point3D() else -1
            ...
            matches, _ = self.get_matches(qname, image.name)
            matches = matches[points3D_ids[matches[:, 1]] != -1]
            num_matches += len(matches)
            for idx, m in matches:
                id_3D = points3D_ids[m]
        ...
        mkp_idxs = [i for i in idxs for _ in kp_idx_to_3D[i]]
        mp3d_ids = [j for i in idxs for j in kp_idx_to_3D[i]]
        ret = self.localizer.localize(
            kpq, mkp_idxs, mp3d_ids, query_camera, **kwargs)
        ret['camera'] = {
            'model': query_camera.model_name,
            'width': query_camera.width,
            'height': query_camera.height,
    ...
    def get_matches(self, name0: str, name1: str):
        pair = names_to_pair(name0, name1)

        matches = self.matches[pair]['matches0']
        scores = self.matches[pair]['matching_scores0']
