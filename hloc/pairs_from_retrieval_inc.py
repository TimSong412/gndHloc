...
class PairRetriever():
    def __init__(self,
        ...
        db_names_h5 = list(name2db.keys())
        images = read_images_binary(db_model / 'images.bin')
        self.db_names = [i.name for i in images.values()]
        self.db_desc = get_descriptors(self.db_names, db_descriptors, name2db)

    def match(self, query_descriptor, query_name) -> dict:
        '''
        input: query feature dict
        '''
        query_names = [query_name]
        query_desc = self.convertdesc(query_des=query_descriptor, query_name=query_name)
        sim = torch.einsum('id,jd->ij', query_desc.to(self.device), self.db_desc.to(self.device))

        selfmatch = np.array(query_names)[:, None] == np.array(self.db_names)[None]
        pairs = pairs_from_score_matrix(sim, selfmatch, self.num_matched, min_score=0)
        pairs = [(query_names[i], self.db_names[j]) for i, j in pairs]

        logger.info(f'Found {len(pairs)} pairs.')
        return pairs

    def convertdesc(self, query_des: dict, query_name):
        desc = [query_des[query_name]['global_descriptor']]
        return torch.from_numpy(np.stack(desc, 0)).float()
