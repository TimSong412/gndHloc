...
class FeatureExtractor():
    ...
    def __init__(self,
        ...
        self.preproc_conf = SimpleNamespace(**{**self.default_conf, **conf['preprocessing']})                    
    ...
    def convertimg(self,
                   npimg: np.ndarray):
        '''
        input: cv2 read img, BGR
        output: data dict
        '''
        if npimg is None:
            raise ValueError('image input error')
        if not self.preproc_conf.grayscale and len(npimg.shape) == 3:
            npimg = npimg[:, :, ::-1]  # BGR to RGB
        if self.preproc_conf.grayscale and len(npimg.shape) == 3:
            npimg = cv2.cvtColor(npimg, cv2.COLOR_BGR2GRAY)

        image = npimg.astype(np.float32)
        size = image.shape[:2][::-1]

        if self.preproc_conf.resize_max and (self.preproc_conf.resize_force
                                     or max(size) > self.preproc_conf.resize_max):
            scale = self.preproc_conf.resize_max / max(size)
            size_new = tuple(int(round(x*scale)) for x in size)
            image = resize_image(image, size_new, self.preproc_conf.interpolation)

        if self.preproc_conf.grayscale:
            image = image[None]
        else:
            image = image.transpose((2, 0, 1))  # HxWxC to CxHxW
        image = image / 255.
    ...
    def imgextract(self, img:np.ndarray, name=None):
        ...
        if 'keypoints' in pred:
            ...
            pred['uncertainty'] = np.array([uncertainty])
...
def main(conf: Dict,
    ...
    for idx, data in enumerate(tqdm(loader, desc=conf['model']['name'])):
